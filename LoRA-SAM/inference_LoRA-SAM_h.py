...
def main():
    ...
    sam = build_sam_vit_h(checkpoint=config_file["SAM"]["CHECKPOINT"])  # Make sure checkpoint points to SAM-H weights
    ...
    print(f"Evaluating LoRA-SAM-H with Rank {rank}...")
    try:
        ...
        csv_file = "./output_data/evaluation_results_sam_h.csv"  # Changed filename
        ...
        plt.bar(x - width/2, baseline_values, width, label='Baseline SAM-H')
        plt.bar(x + width/2, lora_values, width, label=f'LoRA-SAM-H (Rank {rank})')

        plt.xlabel('Metrics')
        plt.ylabel('Score')
        plt.title('Comparison of Baseline SAM-H vs LoRA-SAM-H')
        plt.xticks(x, metrics_names)
        plt.legend()

        os.makedirs("./plots", exist_ok=True)
        plt.savefig("./plots/comparison_plot_sam_h.jpg")  # Changed filename
        print("Plot saved to ./plots/comparison_plot_sam_h.jpg")
